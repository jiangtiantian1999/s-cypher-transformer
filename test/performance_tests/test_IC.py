...
class TestIC(TestCase):
    ...
    def tearDownClass(cls) -> None:
        ...
        results.to_csv(os.path.join("results", "IC_results.csv"))

    # 特定名字的传递朋友
    def test_IC_1(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "IC", "IC_1_records.csv"), index_col=[0, 1])
        ...
        # result_df.to_csv(os.path.join("results", "IC", "IC_1_records.csv"))

    # 朋友的最新评论（仅考虑在给定日期maxDate之前的评论）
    def test_IC_2(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "IC", "IC_2_records.csv"), index_col=[0, 1])
        start_persons = [index[0] for index in result_df.index.values]
        response_time = timedelta()
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "IC", "IC_2_records.csv"))

    # （在[t1,t2]期间内）评论过某个商品的朋友和朋友的朋友
    def test_IC_3(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "IC", "IC_3_records.csv"), index_col=[0, 1])
        start_persons = [index[0] for index in result_df.index.values]
        response_time = timedelta()
        for index, start_person in enumerate(self.start_persons):
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "IC", "IC_3_records.csv"))

    # 新商品：查找朋友在[t1,t2]新评论的商品（t1之前从未评论过），返回这些商品的信息
    def test_IC_4(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "IC", "IC_4_records.csv"), index_col=[0, 1])
        start_persons = [index[0] for index in result_df.index.values]
        response_time = timedelta()
        for index, start_person in enumerate(self.start_persons):
            start_time = self.start_times[index].strftime("\"%Y-%m-%d\"")
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "IC", "IC_4_records.csv"))

    # 新购买：查询朋友的朋友在minDate之后购买过的商品，对于每一个商品，计算顾客评论的数量
    def test_IC_5(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "IC", "IC_5_records.csv"), index_col=[0, 1])
        start_persons = [index[0] for index in result_df.index.values]
        response_time = timedelta()
        for index, start_person in enumerate(self.start_persons):
            start_time = self.start_times[index].strftime("\"%Y-%m-%d\"")
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "IC", "IC_5_records.csv"))

    # 一起购买的商品的标签：在朋友和朋友的朋友购买的商品中查询与某个商品一起购买过的其他商品的标签，返回前10个标签和该标签下商品的数量
    def test_IC_6(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "IC", "IC_6_records.csv"), index_col=[0, 1])
        start_persons = [index[0] for index in result_df.index.values]
        response_time = timedelta()
        for index, start_person in enumerate(self.start_persons):
            s_cypher = "MATCH (n:Customer{id: \"" + start_person + "\"})-[:knows*1..2]->(m:Customer) " + \
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "IC", "IC_6_records.csv"))

    # 最近的评论者，最近20个评论了startPerson购买过的商品的顾客，返回这些顾客的信息，以及该顾客是否为startPerson的朋友
    def test_IC_7(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "IC", "IC_7_records.csv"), index_col=[0, 1])
        start_persons = [index[0] for index in result_df.index.values]
        response_time = timedelta()
        for start_person in self.start_persons:
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "IC", "IC_7_records.csv"))

    # 最近的评论：最近20条对startPerson购买过的商品的所进行的评论
    def test_IC_8(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "IC", "IC_8_records.csv"), index_col=[0, 1])
        start_persons = [index[0] for index in result_df.index.values]
        response_time = timedelta()
        for start_person in self.start_persons:
            s_cypher = "MATCH (n:Customer{id: \"" + start_person + "\"})-[:purchases]->(p:Product)" + \
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "IC", "IC_8_records.csv"))

    # 朋友和朋友的朋友在maxDate之前评论的最近10个商品
    def test_IC_9(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "IC", "IC_9_records.csv"), index_col=[0, 1])
        start_persons = [index[0] for index in result_df.index.values]
        response_time = timedelta()
        for index, start_person in enumerate(self.start_persons):
            end_time = self.end_times[index].strftime("\"%Y-%m-%d\"")
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "IC", "IC_9_records.csv"))
    ...
    def test_IC_11(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "IC", "IC_11_records.csv"), index_col=[0, 1])
        start_persons = [index[0] for index in result_df.index.values]
        response_time = timedelta()
        for index, start_person in enumerate(self.start_persons):
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "IC", "IC_11_records.csv"))

    # 查找朋友在给定标签的商品下的评论，统计每个顾客在每个标签下评论的数量
    def test_IC_12(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "IC", "IC_12_records.csv"), index_col=[0, 1])
        start_persons = [index[0] for index in result_df.index.values]
        response_time = timedelta()
        for index, start_person in enumerate(self.start_persons):
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "IC", "IC_12_records.csv"))

    # 单条最短路径：在给定id的两个顾客之间找到knows的最短顺序路径
    def test_IC_13(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "IC", "IC_13_records.csv"), index_col=[0, 1])
        start_persons = [index[0] for index in result_df.index.values]
        response_time = timedelta()
        for index, start_person in enumerate(self.start_persons):
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "IC", "IC_13_records.csv"))
