...
class TestBI(TestCase):
    ...
    def test_BI_1(self):
        ...
        result_df = pd.read_csv(os.path.join("results", "BI_1_records.csv"), index_col=[0])
        ...
        # result_df.to_csv(os.path.join("results", "BI_1_records.csv"))

    # 查询每个Tag在[minDate, minDate+100]和[minDate+100, minDate+200]两个时间区间内在product中使用的次数，以及次数的差值
    def test_BI_2(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "BI_2_records.csv"), index_col=[0])
        total_record = []
        for start_time in self.start_times:
            ...
            # result_df = pd.concat([result_df, record_df])
        assert total_record == result_df.to_dict("records")
        # result_df.to_csv(os.path.join("results", "BI_2_records.csv"))

    # 查找属于country的用户所购买的所有商品，该商品被tag标记，按照tag计算商品的数量
    def test_BI_3(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "BI_3_records.csv"), index_col=[0, 1])
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "BI_3_records.csv"))

    # 查找各国在minDate之后加入的最活跃的100个顾客（购买商品最多），计算他们对每个购买的商品评论的数量
    def test_BI_4(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "BI_4_records.csv"), index_col=[0, 1])
        countries = [index[0] for index in result_df.index.values]
        for index, country in enumerate(self.countries):
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "BI_4_records.csv"))

    # 查询每个购买了给定标签tag下商品的顾客，根据顾客在这些商品下所发布的评论的数量、votes，helpful计算一个分数
    def test_BI_5(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "BI_5_records.csv"), index_col=[0, 1])
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "BI_5_records.csv"))

    # 查询每个购买了给定标签tag下商品的顾客，查找顾客所购买的这些商品的总销售额
    def test_BI_6(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "BI_6_records.csv"), index_col=[0, 1])
        tags = [index[0] for index in result_df.index.values]
        for tag in self.tags:
            s_cypher = "MATCH (t:Tag{id: " + tag + "})" + \
                       "OPTIONAL MATCH (p:Product)-[:hasTag]->(t) " \
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "BI_6_records.csv"))

    # 查询所有带有给定tag的商品，查询这些商品共同购买的商品的tags，计算每个tag下的共同购买商品的数量
    def test_BI_7(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "BI_7_records.csv"), index_col=[0, 1])
        tags = [index[0] for index in result_df.index.values]
        for tag in self.tags:
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "BI_7_records.csv"))

    # 查询在minDate之后购买过标记给定tag的商品的顾客以及顾客的朋友，计算购买这些顾客购买标记给定tag的商品的数量
    def test_BI_8(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "BI_8_records.csv"), index_col=[0, 1])
        tags = [index[0] for index in result_df.index.values]
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "BI_8_records.csv"))

    # 对于每个顾客，计算他们在[minDate, maxDate]内购买的商品数量，以及这些商品的评论总数
    def test_BI_9(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "BI_9_records.csv"), index_col=[0])
        total_record = []
        for index, start_time in enumerate(self.start_times):
            ...
            # result_df = pd.concat([result_df, record_df])
        assert total_record == result_df.to_dict("records")
        # result_df.to_csv(os.path.join("results", "BI_9_records.csv"))

    # 对于给定顾客，通过kowns关系查找到居住在给定国家，并且距离在2内的所有其他顾客
    # 对于每个每个顾客，查找他们的所购买的所有被tag标记的商品，按照顾客和tag计算商品数量
    def test_BI_10(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "BI_10_records.csv"), index_col=[0, 1])
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "BI_10_records.csv"))

    # 对于给定国家，找出所有在[minDate, maxDate]内互为朋友的三个顾客
    def test_BI_11(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "BI_11_records.csv"), index_col=[0, 1])
        countries = [index[0] for index in result_df.index.values]
        for index, country in enumerate(self.countries):
            start_time = self.start_times[index].strftime("\"%Y-%m-%d\"")
            end_time = self.end_times[index].strftime("\"%Y-%m-%d\"")
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "BI_11_records.csv"))

    # 对于每个顾客，计算他们的评论数量，仅在minDate之后发布，评分大于等于4的评论
    def test_BI_12(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "BI_12_records.csv"), index_col=[0])
        total_record = []
        for start_time in self.start_times:
            start_time = start_time.strftime("\"%Y-%m-%d\"")
            ...
            # result_df = pd.concat([result_df, record_df])
        assert total_record == result_df.to_dict("records")
        # result_df.to_csv(os.path.join("results", "BI_12_records.csv"))

    # 在country中查找僵尸顾客，以及其僵尸分数。僵尸顾客是指在maxDate之前创建的顾客，且每年平均购买不超过1件商品
    # 僵尸分数 = 从其他僵尸哪里获得的knows边数量/总knows边数量
    def test_BI_13(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "BI_13_records.csv"), index_col=[0, 1])
        countries = [index[0] for index in result_df.index.values]
        for index, country in enumerate(self.countries):
            end_time = self.end_times[index].strftime("\"%Y-%m-%d\"")
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "BI_13_records.csv"))
    ...
    def test_BI_14(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "BI_14_records.csv"), index_col=[0, 1])
        countries = [index[0] for index in result_df.index.values]
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "BI_14_records.csv"))
    ...
    def test_BI_16(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "BI_16_records.csv"), index_col=[0, 1])
        tags = [index[0] for index in result_df.index.values]
        for index, tag in enumerate(self.tags):
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "BI_16_records.csv"))

    # startPerson评论一个给定tag的商品过duration后，购买这个商品的其他顾客在其他标记了tag且startPerson未购买过的的商品下进行了评论，返回评论的数量
    def test_BI_17(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "BI_17_records.csv"), index_col=[0, 1])
        start_persons = [index[0] for index in result_df.index.values]
        for index, start_person in enumerate(self.start_persons):
            tag = self.tags[index]
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "BI_17_records.csv"))

    # 为每个购买过Tag标记的商品的顾客推荐新朋友，这个新朋友不认识该顾客，且与该顾客至少有一位共同好友，且同样购买过Tag标记的商品
    def test_BI_18(self):
        # result_df = pd.DataFrame()
        result_df = pd.read_csv(os.path.join("results", "BI_18_records.csv"), index_col=[0, 1])
        tags = [index[0] for index in result_df.index.values]
        for tag in self.tags:
        ...
        #                              columns=keys)
        #     result_df = pd.concat([result_df, record_df])
        # result_df.to_csv(os.path.join("results", "BI_18_records.csv"))
